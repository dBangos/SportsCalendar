# Script that scraps autosport for Formula1 race dates and times
import requests
...
def formula():
    # Loading the data
    page = "https://www.autosport.com/f1/schedule/2024/"
    headers = {'User-Agent':
    ...
    for i in range(len(data)):
        if len(data[i]) == 2:  # Removing cancelled items and adjusting timestamps
            data[i] = ""
            timestamp_offset += 1
            ...
            data[i][0] = "Formula 1 " + data[i][0] + " " + data[i][1]
    ...
    for item in data:
        ...
        item.append("/static/icons/formula_1_logo.png")
    return data